...
PT_THREAD(protothread_vga_debug(struct pt *pt)) {
    ...
    if (!heatmap_precomputed) {
        for (int y = 0; y < HEATMAP_HEIGHT; y++) {
            ...
            for (int x = 0; x < HEATMAP_WIDTH; x++) {
                ...
                // clamp shifts
                if (s_ab < -MAX_SHIFT_SAMPLES) s_ab = -MAX_SHIFT_SAMPLES;
                else if (s_ab > MAX_SHIFT_SAMPLES) s_ab = MAX_SHIFT_SAMPLES;
                if (s_ac < -MAX_SHIFT_SAMPLES) s_ac = -MAX_SHIFT_SAMPLES;
                else if (s_ac > MAX_SHIFT_SAMPLES) s_ac = MAX_SHIFT_SAMPLES;
                if (s_bc < -MAX_SHIFT_SAMPLES) s_bc = -MAX_SHIFT_SAMPLES;
                else if (s_bc > MAX_SHIFT_SAMPLES) s_bc = MAX_SHIFT_SAMPLES;
    ...
    while (true) {
        ...
        for (int i = 1; i < BUFFER_SIZE; ++i) {
            ...
            drawLine(x0 - shift_ab, y0, x1 - shift_ab, y1, BLUE);
            y0 = baseC - (buffer_c.buffer[i-1] >> VERTICAL_SCALE);
            y1 = baseC - (buffer_c.buffer[i]   >> VERTICAL_SCALE);
            drawLine(x0 - shift_ac, y0, x1 - shift_ac, y1, WHITE);
        }
        ...
        // 5) Draw heatmap
        {
            ...
            for (int y = 0; y < h; y++) {
                for (int x = 0; x < w; x++) {
                    ...
                    int64_t L = (int64_t)corr_ab.correlations[idxab]
                                + corr_ac.correlations[idxac]
                                + corr_bc.correlations[idxbc];
            ...
            for (int y = 0; y < h; y++) {
                ...
                int64_t L0 = (int64_t)corr_ab.correlations[idx0]
                            + corr_ac.correlations[heat_idx_ac[y][0]]
                            + corr_bc.correlations[heat_idx_bc[y][0]];
